...
function OutboundFormTracker(tracker, opts) {
  ...
  this.hsdelegate = delegate(document, 'form',
      'hsvalidatedsubmit', this.handleHubspotSubmits.bind(this));
}
...
/**
 * Handles tracking of hubspot forms when dispatched.
 * @param {Event} event The custom hsvalidatedsubmit event.
 */
OutboundFormTracker.prototype.handleHubspotSubmits = function(event) {

  var form = event.delegateTarget;
  var action = form.getAttribute('action');
  var fieldsObj = {transport: 'beacon'};

  this.tracker.send('event', 'HubSpot Form', 'submit', action, fieldsObj);
};
